...
def test_study_cafe_matches_demo_room_and_seat_count(
    ...
    assert world.findtext('scene/ambient') == '0.65 0.65 0.65 1'
    assert world.findtext('scene/background') == '0.82 0.83 0.84 1'
...
def test_walls_use_white_matte_gypsum_material(tmp_path: Path) -> None:
    ...
    for name in ('wall_north', 'wall_south', 'wall_east', 'wall_west'):
        ...
        assert wall.findtext('link/visual/material/ambient') == (
            '0.97 0.97 0.96 1'
        )
        assert wall.findtext('link/visual/material/diffuse') == (
            '0.97 0.97 0.96 1'
        )
