...
def transform_colored_cloud(
    cloud: ColoredPointCloud,
    transform: RigidTransform,
) -> ColoredPointCloud:
    """Express a colored point cloud in the transform destination frame."""
    points = (
        cloud.points.astype(np.float64) @ transform.rotation.T
        + transform.translation
    )
    return ColoredPointCloud(points.astype(np.float32), cloud.colors.copy())
